...
- [```libs/api.py```](libs/api.py) - A library of functions handling with communicating with external API services.
...
2. Fetch files: fetches every html file stored in any of the directories listed in `DEFAULT_DATASTORES` in `settings.py`, and performs some basic analysis on the contents to determine which year the job was posted in.
3. Create tar files: Each job html file found in Step 2 is compressed and added to a tar file containing all jobs released in the same year.  The tar files are in the directory set at `TARPATH` in `settings.py`, and will be created if they do not already exist.  Note that the original html files are NOT deleted by the pipeline in order to prevent mishaps causing loss of data, and these files should be tidied up manually.
4. Extract data from job files: the html files for each job are parsed to extract a number of useful parameters, such as job title, job location and salary range.  This step produces no output.
5. Create database: using the values parsed in stage 3, add the new jobs to a sqlite3 database of jobs (or create the database if it does not exist).  The database is stored at the path provided as `DB_LOCATION` in `settings.py`.  Also creates a db logfile, listing the fields that were collected for the job data, and how many jobs have valid data for each field.
...
* `--no-api` : disables Google Places API functionality when attempting to improve location data in Step 4.
* `--no-scrape` : skips Step 1 of the pipeline.
* `--no-tar` : skips Step 3 of the pipeline
